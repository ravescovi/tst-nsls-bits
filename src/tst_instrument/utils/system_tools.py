...
def generate_device_inventory() -> Dict[str, Any]:
    ...
    try:
        # Iterate through all devices in oregistry
        for device_name in oregistry.keys():
            try:
                ...
                try:
                    if hasattr(device, 'connected'):
                        if callable(device.connected):
                            ...
                            is_connected = device.connected
                        device_info["status"] = "connected" if is_connected else "disconnected"
                ...
                device_info["mock_mode"] = (
                    os.environ.get("TST_MOCK_MODE", "NO") == "YES" or
    ...
    logger.info(f"Generated inventory for {inventory['summary']['total_devices']} devices")
    return inventory
...
def get_available_plans() -> Dict[str, Any]:
    ...
    try:
        ...
        from ..plans import tomography_plans, xas_plans, sim_plans, dm_plans
                    ...
                    if pv_prefix:
                        ...
                        try:
                            if hasattr(device, 'connected'):
                                ...
                                connection_info["status"] = "connected" if is_connected else "disconnected"
...
def benchmark_performance() -> Dict[str, Any]:
    ...
    try:
        ...
        device_count = len(oregistry.keys())
        end_time = time.time()
    ...
    try:
        start_time = time.time()
        test_devices = list(oregistry.keys())[:5]  # Test first 5 devices
    ...
    try:
        ...
        from .hardware import warmup_hdf5_plugins
    ...
    benchmarks["summary"]["total_tests"] = benchmarks["summary"]["passed_tests"] + benchmarks["summary"]["failed_tests"]
    
    if response_times:
        benchmarks["summary"]["average_response_time"] = sum(response_times) / len(response_times)
        benchmarks["summary"]["max_response_time"] = max(response_times)
        benchmarks["summary"]["min_response_time"] = min(response_times)
    
    logger.info(f"Performance benchmarks complete: {benchmarks['summary']['passed_tests']}/{benchmarks['summary']['total_tests']} passed")
    return benchmarks
...
def generate_system_report() -> Dict[str, Any]:
    ...
    try:
        ...
        report["summary"] = {
            ...
            "connected_devices": report["device_inventory"]["summary"]["connected_devices"],
            "total_plans": report["plan_information"]["summary"]["total_plans"],
            "connected_pvs": report["pv_validation"]["summary"]["connected_pvs"],
            "benchmark_score": f"{report['performance_benchmarks']['summary']['passed_tests']}/{report['performance_benchmarks']['summary']['total_tests']}"
        }
