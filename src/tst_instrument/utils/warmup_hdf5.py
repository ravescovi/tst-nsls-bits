...
def warmup_hdf5_plugins(detectors: List[Any]) -> None:
    ...
    for det in detectors:
        try:
            ...
            needs_warmup = (
                hasattr(_array_size, "height")
                and hasattr(_array_size, "width")
                and (0 in [_array_size.height, _array_size.width])
            )

            if needs_warmup:
                logger.info(
                    f"Warming up HDF5 plugin for {det.name} - array_size={_array_size}"
                )
                print(
                ...
                try:
                    ...
                    logger.info(
                        f"HDF5 warmup completed for {det.name} - new array_size={new_array_size}"
                    )
                    print(
                        f"  Warming up HDF5 plugin for {det.name} is done. array_size={new_array_size}\n"
                    )
                    warmed_count += 1
                    ...
                    print(
                        f"  ERROR: Failed to warm up HDF5 plugin for {det.name}: {e}\n"
                    )

            else:
                logger.debug(
                    f"HDF5 warmup not needed for {det.name} - array_size={_array_size}"
                )
                print(
            ...
            logger.error(
                f"Error processing detector {getattr(det, 'name', 'unknown')}: {e}"
            )
            skipped_count += 1

    logger.info(
        f"HDF5 warmup completed: {warmed_count} warmed, {skipped_count} skipped"
    )
    print(
