...
def xas_demo_async(
    ...
    # Create flyers dynamically (like in original TST)
    # For now, we'll create basic flyer placeholders
    # In real implementation, these would be properly configured
    manta_flyer = None  # Will be created dynamically in actual use
    panda_flyer = None  # Will be created dynamically in actual use
    ...
    if detector:
        ...
        print("Manta preparation complete:", datetime.datetime.now().strftime("%H:%M:%S"))
    
    yield from bps.prepare(panda_flyer, npoints, wait=True)
    yield from bps.prepare(panda, panda_flyer.trigger_info, wait=True)
    print("PandA preparation complete:", datetime.datetime.now().strftime("%H:%M:%S"))
