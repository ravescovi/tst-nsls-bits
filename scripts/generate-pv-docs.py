...
try:
    from tst_instrument.utils.system_tools import generate_device_inventory, validate_pv_connections
    from apsbits.core.instrument_init import oregistry
except ImportError as e:
    print(f"Error importing TST instrument modules: {e}")
    print("Make sure you're running this script from an activated development environment")
    sys.exit(1)
            ...
            try:
                if hasattr(device, 'connected'):
                    if callable(device.connected):
                        ...
                        is_connected = device.connected
                    device_pvs["status"] = "connected" if is_connected else "disconnected"
            except Exception:
...
def generate_pv_documentation(output_path: Path, include_validation: bool = True) -> None:
    """
    ...
    with open(output_path, 'w') as f:
        ...
        # Summary section
        f.write("## Summary\n\n")
        f.write(f"- **Total Devices**: {device_inventory['summary']['total_devices']}\n")
        f.write(f"- **Connected Devices**: {device_inventory['summary']['connected_devices']}\n")
        f.write(f"- **Mock Mode**: {device_inventory['devices'].get(list(device_inventory['devices'].keys())[0], {}).get('mock_mode', 'unknown')}\n")
        
        if validation_results:
            f.write(f"- **Total PVs**: {validation_results['summary']['total_pvs']}\n")
            f.write(f"- **Connected PVs**: {validation_results['summary']['connected_pvs']}\n")
            f.write(f"- **Disconnected PVs**: {validation_results['summary']['disconnected_pvs']}\n")
        ...
        for device_name, info in pv_info.items():
            ...
            if info.get('components'):
                ...
                for comp_name, comp_info in info['components'].items():
                    f.write(f"| {comp_name} | `{comp_info['prefix']}` | {comp_info['type']} |\n")
                f.write("\n")
        ...
        if validation_results:
            ...
            f.write("|--------|----- |\n")
            f.write(f"| Connected | {validation_results['summary']['connected_pvs']} |\n")
            f.write(f"| Disconnected | {validation_results['summary']['disconnected_pvs']} |\n")
            f.write(f"| Failed Checks | {validation_results['summary']['failed_checks']} |\n")
            f.write("\n")
            ...
            for device_name, conn_info in validation_results['connections'].items():
                ...
                f.write(f"- **{device_name}** {status_icon} `{conn_info.get('pv_prefix', 'N/A')}` - {conn_info['status']}")
        ...
        f.write("```console\n")
        f.write(f"TST_MOCK_MODE: {device_inventory['devices'].get(list(device_inventory['devices'].keys())[0], {}).get('mock_mode', 'unknown')}\n")
        f.write("```\n\n")
        
        # Footer
        f.write("---\n\n")
        f.write("*This documentation was automatically generated by the TST NSLS-II BITS system.*\n")
        f.write(f"*For more information, see the [TST BITS Repository](https://github.com/ravescovi/tst-nsls-bits).*\n")
